...
file1 = open(path+"/"+path+" Data Stitched 6-24-21.csv", "w")
file2 = open(path+"/"+path+" Data Clocklab PIM only 6-24.AWD", "w")
file2.write(path+"\n")
...
for name in fileList:
    ...
    # if the data does not start at 12:00 on the start day, fill the space with 0s
        ...
        if((df["DATE"][0] == sdslash) | (df["DATE"][0] == sdslash2)):
            ...
            blankdf[["PIM"]].to_csv(file2, index=False, header=None)
        else:
    ...
    df[["PIM"]][startrow:endrow].to_csv(file2, index=False, header=None)
